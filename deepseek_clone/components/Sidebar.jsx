...
const Sidebar = ({expand,setexpand}) => {
    ...
    const  {user,chats,createNewChat} = useAppContext(AppContext)
    const [openMenu,setopenMenu] = useState({id:0,open:false})

  return (
    <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-md:absolute max-md:h-screen ${expand ? 'p-4 w-64' : 'md:w-20 w-0  max-ms:overflow-hidden'}`}>
      <div>
        <div onClick={()=>expand ? setexpand(false):setexpand(true)} className={`flex ${expand ? "flex-row gap-10":"flex-col items-center gap-8"}`}>
            ...
            <div className='group relative flex items-center justify-center hover:bg-gray-500/20 transition-all duration-300 h-9 w-9 aspect-square rounded-lg cursor-pointer'>
                ...
                <div className={`absolute w-max ${expand ? "left-1/2  -translate-x-1/2 top-12":"-top-12 left-0"} opacity-0 group-hover:opacity-100 transition bg-black text-white text-sm px-3 py-2 rounded-lg shadow-lg pointer-evwnts-none`}>
        ...
        <button onClick={createNewChat} className={`mt-8 flex items-center justify-center cursor-pointer  ${expand?"bg-primary hover:opacity-90 rounded-2xl gap-2 p-2.5 w-max":"group relative h-9 mx-auto hover:bg-gray-500/30 rounded-lg"}`}>
            <Image src={expand ? assets.chat_icon : assets.chat_icon_dull} alt='' className={expand ? 'w-6' : 'w-7'} />
            <div className='absolute w-max -top-12 -right-12 opacity-0 group-hover:opacity-100 transition bg-black text-white txet-sm  px-3 py-2 rounded-lg shadow-lg pointer-events-none'>
                New Chat 
      ...
      <div>
        ...
        <div onClick={user ? null : openSignIn} className={`flex items-center ${expand ? "hover:bg-white/10 rounded-lg":"justify-center w-full"} gap-3 text-white/50 text-sm p-2 mt-2 cursor-pointer`}>
            {
